...
public class FindTrends {
    ...
    private static HashMap<String, int[]> trendValues1 = new HashMap<String, int[]>();
    private static HashMap<String, Integer> trendValues2 = new HashMap<String, Integer>();
    // the text file that the results will be printed to
    private static File outputFile;
    private static int totalNumDays;
    private static int currDayNum;
	...
	private static void calcDay(String day, int sign) {
		...
		try {
			...
			while (rs.next()) {
				...
				count *= sign;
				if (trendValues1.get(link) == null) {
					int[] tempArr = new int[totalNumDays];
					tempArr[currDayNum] = count;
					trendValues1.put(link, tempArr);
				} else {
					int[] tempArr = new int[totalNumDays];
					tempArr = trendValues1.get(link);
					tempArr[currDayNum] = count;
					trendValues1.put(link, tempArr);
				}
			}
	...
	private static void setTotalNumDays(String startDate, String endDate) {
		SimpleDateFormat sdf = new SimpleDateFormat("MM-dd-yyyy");
		Calendar c1 = Calendar.getInstance();
		Calendar c2 = Calendar.getInstance();
		try {
		...
		totalNumDays = (int) ((c2.getTimeInMillis() - c1.getTimeInMillis()) / 79200000);
	}
	...
	private static SortedSet<Map.Entry<String, Integer>> entriesSortedByValues() {
	    ...
	    sortedEntries.addAll(trendValues2.entrySet());
	    System.out.println("Sorted values");
	    return sortedEntries;
	}
	
	private static void convert() {
		for (Entry<String, int[]> e : trendValues1.entrySet()) {
			int[] array = e.getValue();
			String key = e.getKey();
			int maxIndex = 0;
			int max = 0;
			int sum = 0;
